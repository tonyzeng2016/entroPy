...
#define EXTRACT_CONTENT_FROM_UNICODE_STR(str,item) do{ \
    if(item&&PyUnicode_Check(item)) str=PyString_AsString(PyUnicode_AsUTF8String(item));\
    else if(item&&PyString_Check(item))str=PyString_AsString(item);}while(0)
...
static PyObject* MaxEntTrainer_append_instance(MaxEntTrainer *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "OO",&items,&labelItem))
        return NULL;
    ///判断非list退出
    EXTRACT_CONTENT_FROM_UNICODE_STR(label,labelItem);
...
/////////////////////////////////////////////////////////////////////////////
    for(i=0; i<itemsLen; i++)
    {
        ...
        EXTRACT_CONTENT_FROM_UNICODE_STR(itemStr,item);
...
static PyObject* MaxEntTrainer_save_model(MaxEntTrainer *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|O",&item))
        return NULL;
    EXTRACT_CONTENT_FROM_UNICODE_STR(fileName,item);
